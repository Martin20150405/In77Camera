...
//    color = NCColorControl(color, 0.6, -0.06, 0.75);
    ...
    // rand
	  float x = vTextureCoord.x*inputImageTextureWidth/texture2Size;
      float y = vTextureCoord.y*inputImageTextureHeight/texture2Size;

//      vec4 rd = texture2D(sTexture2, vec2( fract(x), fract(y)));
    vec4 rd = texture2D(sTexture2, vTextureCoord);
