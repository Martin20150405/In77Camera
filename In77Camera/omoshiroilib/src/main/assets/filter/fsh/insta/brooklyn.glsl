 ...
 vec4 NCTonemapping(vec4 color)
{
    ...
    vec4 mapped;
    mapped.r = texture2D(sTexture2, vec2(color.r, 0.0)).r;
    mapped.g = texture2D(sTexture2, vec2(color.g, 0.0)).g;
    mapped.b = texture2D(sTexture2, vec2(color.b, 0.0)).b;
    mapped.a = color.a;
 ...
 void main()
{
    ...
    // tone mapping
    color.r = texture2D(sTexture2, vec2(color.r, 0.0)).r;
    color.g = texture2D(sTexture2, vec2(color.g, 0.0)).g;
    color.b = texture2D(sTexture2, vec2(color.b, 0.0)).b;
    ...
    // normal blend
    vec4 bg2 = texture2D(sTexture3, vTextureCoord);
    bg2.a *= 0.9;
    color = NCMultiplyBlend(bg2, color);
    
    // tone mapping
    color.r = texture2D(sTexture4, vec2(color.r, 0.0)).r;
    color.g = texture2D(sTexture4, vec2(color.g, 0.0)).g;
    color.b = texture2D(sTexture4, vec2(color.b, 0.0)).b;
