...
uniform lowp sampler2D sTexture2; //curve
uniform lowp sampler2D sTexture3; //mask
...
{ 
	vec4 color = texture2D(sampler, vTextureCoord) * 2.;
	
	color -= texture2D(sampler, vTextureCoord-vec2(texelWidthOffset, 0. )) * 0.25;
	color -= texture2D(sampler, vTextureCoord+vec2(texelWidthOffset, 0. )) * 0.25;
	color -= texture2D(sampler, vTextureCoord-vec2(0., texelHeightOffset)) * 0.25;
	color -= texture2D(sampler, vTextureCoord+vec2(0., texelHeightOffset)) * 0.25;
...
{ 
	...
	color += texture2D(sampler,vTextureCoord)* 0.0443 ;
	
	step.x = 1.49583 * texelWidthOffset  * strength; 
	step.y = 1.49583 * texelHeightOffset * strength; 
	color += texture2D(sampler,vTextureCoord+vec2(step.x, 0.)) * 0.04321;
	color += texture2D(sampler,vTextureCoord-vec2(step.x, 0.)) * 0.04321;
	color += texture2D(sampler,vTextureCoord+vec2(0., step.y)) * 0.04321;
	color += texture2D(sampler,vTextureCoord-vec2(0., step.y)) * 0.04321;
	
	step.x = 2.4719250988753685 * texelWidthOffset  * strength; 
	step.y = 2.4719250988753685 * texelHeightOffset * strength; 
	color += texture2D(sampler,vTextureCoord+step) * 0.041795;
	color += texture2D(sampler,vTextureCoord-step) * 0.041795;
	color += texture2D(sampler,vTextureCoord+vec2(-step.x, step.y)) * 0.041795;
	color += texture2D(sampler,vTextureCoord+vec2( step.x,-step.y)) * 0.041795;
	
	step.x = 5.49583 * texelWidthOffset  * strength; 
	step.y = 5.49583 * texelHeightOffset * strength; 
	color += texture2D(sampler,vTextureCoord+vec2(step.x, 0.)) * 0.040425;
	color += texture2D(sampler,vTextureCoord-vec2(step.x, 0.)) * 0.040425;
	color += texture2D(sampler,vTextureCoord+vec2(0., step.y)) * 0.040425;
	color += texture2D(sampler,vTextureCoord-vec2(0., step.y)) * 0.040425;

	step.x = 5.300352223621558 * texelWidthOffset  * strength; 
	step.y = 5.300352223621558 * texelHeightOffset * strength; 
	color += texture2D(sampler,vTextureCoord+step) * 0.0391;
	color += texture2D(sampler,vTextureCoord-step) * 0.0391;
	color += texture2D(sampler,vTextureCoord+vec2(-step.x, step.y)) * 0.0391;
	color += texture2D(sampler,vTextureCoord+vec2( step.x,-step.y)) * 0.0391;

	step.x = 9.49583 * texelWidthOffset  * strength; 
	step.y = 9.49583 * texelHeightOffset * strength; 
	color += texture2D(sampler,vTextureCoord+vec2(step.x, 0.)) * 0.037815;
	color += texture2D(sampler,vTextureCoord-vec2(step.x, 0.)) * 0.037815;
	color += texture2D(sampler,vTextureCoord+vec2(0., step.y)) * 0.037815;
	color += texture2D(sampler,vTextureCoord-vec2(0., step.y)) * 0.037815;
	
	step.x = 8.128779348367749 * texelWidthOffset  * strength; 
	step.y = 8.128779348367749 * texelHeightOffset * strength; 
	color += texture2D(sampler,vTextureCoord+step) * 0.03658;
	color += texture2D(sampler,vTextureCoord-step) * 0.03658;
	color += texture2D(sampler,vTextureCoord+vec2(-step.x, step.y)) * 0.03658;
	color += texture2D(sampler,vTextureCoord+vec2( step.x,-step.y)) * 0.03658;
...
{ 
	...
	if (lowPerformance == 1) 
	{ 	
		c1 = texture2D(sTexture, vTextureCoord);
		c2 = texture2D(sTexture, vTextureCoord);
    } 
	else 
	{ 
		c1 = sharpen(sTexture);
		c2 = normal(c1, gaussianBlur(sTexture), 0.8); // radius = 13. sharpen?? gaussian blur? ???? ??, ?? blending?? ??
	} 
	vec4 c3 = normal(c1, lighten(c1,c2), 0.6); // lighten (0.6) 
    c3 = adjust(c3, 0.12, 0., 0.05); // brightness = 12, saturation = 0.5; 
    c3 = vibrance(level(c3, sTexture2), 0.5); // vibrance = 0.5;
	c3 = normal(c3, overlay(c3, vec4(0.)), 1.-texture2D(sTexture3, vTextureCoord).r); // vignetting
